...
def verify_vector(username, vector):
    ...
    return True if shortest<3*threshold else False
...
def train(entry, parent):
    ...
    current_ku=[]
    if(trained >9):
        tbutton=tk.Button(parent, text="register", command=lambda: save(usr.get(),parent))
        tbutton.grid(row=3, column=1, sticky=tk.EW, padx=10,pady=10)
    return
...
def transform(vector1, vector2):
    print("debug:",len(vector1))
    print("debug:",len(vector2))
    global pwd
    result=[]
...
def register():
    ...
    tk.Message(regscr, text='').grid(row=0,columnspan=5)
    tk.Label(regscr, text="Enter Username").grid(row=1, column=0,columnspan=1)
    usr = tk.Entry(regscr)
    usr.grid(row=1, column=1, columnspan=3, padx=50)
    tk.Label(regscr, text="Enter password").grid(row=2, column=0, columnspan=1)
    regpassword = tk.Entry(regscr)
    regpassword.grid(row=2, column=1, columnspan=3, padx=50)
    regpassword.bind('<KeyPress>',keyd)
    regpassword.bind('<KeyRelease>',keyu)
    tbutton = tk.Button(regscr, text="Train", command=lambda: train(regpassword, regscr))
    tbutton.grid(row=3, column=1, sticky=tk.EW, padx=10,pady=10)
    tmsg = tk.Label(regscr, text="Trainings completed (out of 10):"+str(trained))
    tmsg.grid(row=4, column=0,columnspan=5, sticky=tk.EW)
    regscr.mainloop()
    return
...
def login_screen():
    screen = tk.Tk()
    screen.title("Login")
    tk.Message(screen, text='').grid(row=0,columnspan=10)
    tk.Label(screen, text="Enter Username").grid(row=1, column=0,columnspan=1)
    username = tk.Entry(screen)
    username.grid(row=1, column=1, columnspan=3, padx=50)
    tk.Label(screen, text="Enter password").grid(row=2, column=0, columnspan=1)
    ...
    tk.Button(screen, text="Authenticate", command= lambda: authenticate(username.get(),password.get(),screen)).grid(row=5, column=1, sticky=tk.EW, padx=10,pady=10)
    tk.Label(screen, text='Not registered?').grid(row=6,column=0,sticky=tk.E, columnspan=1, pady=1)
    tk.Button(screen, text="Register", command=register).grid(row=6, column=1, sticky=tk.N, pady=10)
    screen.mainloop()
    return
...
def reg_failure_screen(screen):
    ...
    tk.Message(regfail, text='Registration Failed').grid(row=0, column=0, padx=50, pady=10)
    tk.Button(regfail, text='Quit', command= regfail.destroy).grid(row=2, column=0, padx=50, pady=10)
    regfail.mainloop
    return
